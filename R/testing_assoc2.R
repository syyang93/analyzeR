#' Function that tests for associations and returns results.  DOES NOT merge with gene info, for use with tpm datasets.
...
#' 
#' @examples
#' lm_results <- testing_assoc2(lm_full)
 
testing_assoc2 <- function (lm_full, to_correct = "+ as.factor(covariates$SEX) + as.numeric(covariates$AGE)+ as.numeric(covariates$RACE) + covariates$PC1 + covariates$PC2 +covariates$PC3+ covariates$PC4 + covariates$PC5 + covariates$PC6 + covariates$PC7 + covariates$PC8 + covariates$PC9 + covariates$PC10")
{
