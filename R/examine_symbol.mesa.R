...
examine_symbol.mesa <- function(lm_data, symbol, gene_key, omit.outlier = T, correct_for = 'as.factor(lm_data$SEX) + as.numeric(lm_data$AGE) + as.numeric(lm_data$RACE) +
  ...
  indiv <- nrow(lm_data)
  require(ggplot2)
  index <- grep(paste0('^', symbol, '$'), gene_key$hgnc_symbol)
  if(length(index) == 0) {print('Symbol not found in dataframe')}
  transcript <- gene_key$original_gene[index]
  ...
  if(is.na(col))
  {
  	g <- ggplot(resids, aes(x=CN, y=transcript))+geom_point()+geom_smooth(method="lm", formula=y~x) +xlab('mtDNA-CN') + ylab(paste0('residuals for ', symbol))
  } else{ g <- ggplot(resids, aes(x=CN, y=transcript, col = col))+geom_point()+geom_smooth(method="lm", formula=y~x) +xlab('mtDNA-CN')+ ylab(paste0('residuals for ', symbol))
        }
  return(g)
}
